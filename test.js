...
test('Merge objects and stacks function properties', t => {
  const { mergeObject } = utils;
  const hash = { brown: 'breakfast' };
  const objectString = mergeObject({}, 'string', 10, [], hash);
  t.deepEqual(objectString, hash, 'Does not merge non-object arguments');

  let objectFunction = { sing() { this.lyrics = 'Hello,'; } };
  let anotherObjectFunction = { sing() { this.lyrics += " it's me";  } };
  const mergedObjectFunctions = mergeObject({}, objectFunction, anotherObjectFunction);
  mergedObjectFunctions.sing();
  t.deepEqual(mergedObjectFunctions.lyrics, "Hello, it's me", 'Stacks function properties');
});
